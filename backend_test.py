...
def test_image_editor_save(token=None, collection_id=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "id" in created_item:
                item_id = created_item["id"]
                print_result("Create test jewelry item", True, f"Successfully created jewelry item with ID: {item_id}")
                
                # Test POST /api/save-edited-image for a jewelry item
                edited_item_image_data = {
                    "item_id": item_id,
                    "image_base64": "data:image/jpeg;base64,"
                }
                ...
                if item_response.status_code == 200:
                    ...
                    if collection_response.status_code == 200:
                        ...
                        test_results["image_editor"]["message"] = "Image editor save functionality works for both collections and jewelry items"
                    else:
                    ...
                    test_results["image_editor"]["success"] = False
                    test_results["image_editor"]["message"] = f"Failed to save edited image for jewelry item: {item_response.status_code}"
                ...
                if delete_response.status_code == 200:
                    print_result("Delete test jewelry item", True, "Successfully deleted test jewelry item")
                else:
                    print_result("Delete test jewelry item", False, f"Failed to delete test jewelry item: {delete_response.status_code}")
            else:
                print_result("Create test jewelry item", False, "No ID in created jewelry item response")
                test_results["image_editor"]["success"] = False
                test_results["image_editor"]["message"] = "Failed to create test jewelry item for image editor testing"
        else:
            print_result("Create test jewelry item", False, f"Failed with status code: {response.status_code}")
            test_results["image_editor"]["success"] = False
            test_results["image_editor"]["message"] = f"Failed to create test jewelry item: {response.status_code}"
    except Exception as e:
        print_result("Image Editor Save", False, f"Exception: {str(e)}")
        test_results["image_editor"]["success"] = False
        test_results["image_editor"]["message"] = f"Exception during image editor testing: {str(e)}"
