            ...
            if "id" in created_item:
                ...
                if verify_response.status_code == 200:
                    ...
                    for item in items:
                        if item["id"] == item_id:
                            item_exists = True
                            print(f"Found item in response: {json.dumps(item)}")
                            break
                    
                    if item_exists:
                        print("Jewelry item creation verified")
                        
                        # Wait a moment to ensure the item is fully saved in the database
                        print("Waiting for 2 seconds to ensure item is saved...")
                        time.sleep(2)
